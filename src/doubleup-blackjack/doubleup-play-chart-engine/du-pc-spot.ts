import { DuPlayChartHand } from './du-pc-hand';
import { DuCard } from '../doubleup-engine/du-card';

export class DuPlayChartSpot {
  hands: DuPlayChartHand[] = [];
  totalWon: number = 0;
  ...
  addHand(isFromSplit: boolean = false): void {
    this.hands.push(new DuPlayChartHand(this.shared, 100, isFromSplit, this.conditions));
  }
  ...
  initializeRound() {
    this.totalWon = 0;
    this.hands.push(new DuPlayChartHand(this.shared, 100, false, this.conditions));
  }
