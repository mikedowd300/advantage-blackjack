import { DuPlayChartTable } from './du-pc-table';
...
import { DoubleDownOn, HoleCardType, SurrenderTypes } from '../doubleup-models/doubleup-strategies.models';

export class DuPlayChartEngine {

  table: DuPlayChartTable;
  localStorageService: LocalStorageService = new LocalStorageService();
  pcDataService: DuPlayChartDataStorageService = new DuPlayChartDataStorageService(this.localStorageService);
  shoe: DuShoe;
  ...
  startSimulation(iterations: number, details: string, first2Cards: string) {
    ...
    this.dissectDetails();
    this.shoe = new DuShoe(this.shoeConditions, this.localStorageService);
    this.table = new DuPlayChartTable(
      this.pcDataService,
