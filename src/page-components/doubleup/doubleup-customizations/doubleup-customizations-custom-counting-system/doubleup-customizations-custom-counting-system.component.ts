...
@Component({
  selector: 'doubleup-customizations-custom-counting-system',
  standalone: true,
  imports: [ABJStrategySelectorComponent, ABJTooltipComponent, CommonModule, FormsModule],
  templateUrl: './doubleup-customizations-custom-counting-system.component.html',
  styleUrl: './doubleup-customizations-custom-counting-system.component.scss'
})
export class DoubleupCustomizationsCustomCountingSystemComponent implements OnInit {
  title: string = "Add, Edit or Delete a Custom Counting System";
  defaultStrategy: CountingMethod = { ...doubleupDefaultCount };
  activeStrategy$: BehaviorSubject<CountingMethod> = new BehaviorSubject<CountingMethod>(doubleupDefaultCount);
  activeStrategy: CountingMethod = { ...doubleupDefaultCount };
  localStorageItemsEnum = LocalStorageItemsEnum;
  localStorageVariationKeys = LocalStorageVariationKeys;
  strategies: { [k: string]: CountingMethod } = doubleupCounts;
  strategyTitles: string[] = doubleupCountTitles;
