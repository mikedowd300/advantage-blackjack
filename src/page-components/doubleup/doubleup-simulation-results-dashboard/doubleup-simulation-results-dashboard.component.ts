...
@Component({
  selector: 'doubleup-simulation-results-dashboard',
  standalone: true,
  imports: [
    FormsModule,
    ABJAnchorComponent,
  ...
  templateUrl: './doubleup-simulation-results-dashboard.component.html',
  styleUrl: './doubleup-simulation-results-dashboard.component.scss'
})

export class DoubleupSimulationResultsDashboardComponent implements OnInit {
  handles: string[];
  activeHandle: string;
  tippedAway: number = 0;
  ...
  goToHandReview() {
    ...
    this.router.navigate(['doubleup/hand-review']);
  }
}